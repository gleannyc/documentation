...
<Callout type="info">
  **Row count metric is required**— You can't remove the row count metric from a
  data model in Glean (at least, not very easily). It's good practice to rename
  the row count metric to describe what each row in the table represents (eg.
  Calls). If each row really doesn't mean anything, you can rename in a way that
...
<Callout>
  **Next Steps**- If you're ready to begin building in Glean, learn how to [add
  a Data Model](/data-modeling/add-data-model).
</Callout>
